        ...
        try {
            client.reply (id, buildDict (
                capabilities: buildDict (
                    ...
                    renameProvider: 
                        init_params.capabilities.textDocument.rename.prepareSupport 
                            ? buildDict (prepareProvider: new Variant.boolean (true))
        ...
        wait_for_context_update (id, request_cancelled => {
            ...
            foreach (var btarget_w_sym in SymbolReferences.get_compilations_using_symbol (selected_project, symbol))
                foreach (Vala.SourceFile project_file in btarget_w_sym.first.code_context.get_source_files ()) {
                    ...
                    if (file in generated_vapis || file in shown_files)
                        continue;
                    var file_references = new ArrayList<Vala.CodeNode> ();
                    ...
                    references.add_all (file_references);
                    shown_files.add (file);
                }
            ...
            foreach (var code_node in references) {
                debug ("[%s] editing reference %s @ %s ...", 
                    method, 
                    CodeHelp.get_code_node_representation (code_node), 
                ...
                var range = SymbolReferences.get_replacement_range (code_node, symbol);
        ...
        wait_for_context_update (id, request_cancelled => {
            ...
            foreach (var btarget_w_sym in SymbolReferences.get_compilations_using_symbol (selected_project, symbol)) {
                if (!(btarget_w_sym.second.source_reference.file is TextDocument)) {
                    ...
                    client.reply_error_async.begin (
                        id, 
                        Jsonrpc.ClientError.INVALID_REQUEST, 
                        "Cannot rename a symbol from a system library.", 
                        cancellable);
                    Vala.CodeContext.pop ();
                    return;
                }
            ...
            try {
                client.reply (
                    id,
                    buildDict (
                        range: Util.object_to_variant (SymbolReferences.get_replacement_range (initial_result, symbol)),
